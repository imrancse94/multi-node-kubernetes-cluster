...
We need 3 VM (In AWS EC2 instance)
    1. Master node
    2. Worker node 1
    3. Worker node 2

Here we will use virtualbox to implement k8s cluster.

![alt text](https://github.com/imrancse94/multi-node-kubernetes-cluster/blob/main/images/image1.webp?raw=true)
...
### Step 8: Install Docker (All hosts)
```bash
sudo apt-get install docker.io -y
```

#### Step 9: Access Repos via HTTPS (All hosts)
```bash
...
### Step 10: Initialize the Cluster and join worker accoring to instructions.
...
### Step 11: Download calico.yaml and uncomment IP by searching ```CALICO_IPV4POOL_CIDR``` in calico.yaml file

```bash
curl https://raw.githubusercontent.com/projectcalico/calico/v3.29.1/manifests/calico.yaml -O
```
### Step 12: Apply the calico.yaml
